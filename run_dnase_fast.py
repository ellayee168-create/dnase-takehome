...
OPTIMIZED VERSION - processes chromosomes individually to avoid massive temp files.
...
Usage:
    python run_dnase_fast.py

    # With pre-downloaded BAMs (skip download):
    python run_dnase_fast.py --skip-download

    # Specify cell lines:
    python run_dnase_fast.py --cell-lines GM12878 HeLa-S3
...
# =============================================================================
# BAM to BigWig Conversion (ChromBPNet methodology) - OPTIMIZED
# =============================================================================
...
def bam_to_bigwig_fast(bam_path, output_bw, valid_chroms, chrom_sizes):
    """
    Convert BAM to base-resolution BigWig for DNase-seq.
    
    OPTIMIZED: Uses pysam to read BAM directly, processes chromosome-by-chromosome
    to avoid creating massive temp files.
    ...
    logging.info(f"    Converting BAM to BigWig (optimized, chromosome-by-chromosome)...")

    # Index BAM if needed
    bai_path = bam_path + ".bai"
    if not os.path.exists(bai_path):
        logging.info(f"    Indexing BAM...")
        pysam.index(bam_path)

    bam = pysam.AlignmentFile(bam_path, "rb")

    # Get chromosomes present in BAM that are also in our valid set
    bam_chroms = set(bam.references)
    chroms_to_process = sorted(valid_chroms & bam_chroms & set(chrom_sizes.keys()))
    ...
    for chrom in chroms_to_process:
        ...
        try:
            for read in bam.fetch(chrom):
                # Skip unmapped, secondary, supplementary (matching ChromBPNet behavior)
                if read.is_unmapped or read.is_secondary or read.is_supplementary:
                    continue

                # Get 5' end position with strand-specific shift
                if read.is_reverse:
                    # Minus strand: 5' end is at reference_end (0-based exclusive -> 0-based inclusive is end-1)
                    # But we want the cut site, which is end + MINUS_SHIFT_DELTA
                    pos = read.reference_end + MINUS_SHIFT_DELTA
                else:
                    # Plus strand: 5' end is at reference_start
                    pos = read.reference_start + PLUS_SHIFT_DELTA
            ...
            # Chromosome not in BAM index
            continue

        # Convert to intervals (only non-zero positions)
        if counts.sum() > 0:
            intervals_by_chrom[chrom] = array_to_intervals(counts)
            logging.info(f"      {chrom}: {int(counts.sum())} cuts")

    bam.close()
    logging.info(f"    Total reads processed: {total_reads}")

    # Write BigWig directly using pyBigWig
    logging.info(f"    Writing BigWig...")
...
def bam_to_bigwig(bam_path, output_prefix, valid_chroms):
    ...
    Calls the optimized bam_to_bigwig_fast function.
    """
    output_bw = f"{output_prefix}_unstranded.bw"
    chrom_sizes = load_chrom_sizes()
    return bam_to_bigwig_fast(bam_path, output_bw, valid_chroms, chrom_sizes)

# =============================================================================
# BigWig Writing Utilities
# =============================================================================
...
def process_cell_line(cell_line, skip_download=False, chrombpnet_commit=None):
    ...
    try:
        ...
        for file_info in file_infos:
            ...
            bw_path = os.path.join(bigwig_dir, f"{file_acc}_unstranded.bw")
            
            # Use optimized BAM to BigWig conversion
            bam_to_bigwig_fast(bam_path, bw_path, valid_chroms, chrom_sizes)
...
def main():
    parser = argparse.ArgumentParser(
        description="DNase preprocessing pipeline following AlphaGenome methodology (OPTIMIZED)"
    )
    parser.add_argument(
        "--cell-lines",
        nargs="+",
